...
import pytest
from fixtures.benchmark_fixture import SerenDBBenchmarker, PgBenchRunResult
from fixtures.log_helper import log
from fixtures.serendb_api import SerenDBAPI, connstr_to_env

from performance.test_perf_pgbench import utc_now_timestamp

if TYPE_CHECKING:
    from fixtures.compare_fixtures import SerenDBCompare
    from fixtures.serendb_fixtures import Endpoint, PgBin
...
def test_compare_prewarmed_pgbench_perf(serendb_compare: SerenDBCompare):
    env = serendb_compare.env
    env.create_branch("prewarmed")
    pg_bin = serendb_compare.pg_bin
    ep_ordinary: Endpoint = serendb_compare.endpoint
    ...
    for ep in [ep_ordinary, ep_prewarmed]:
        ...
        ep.safe_psql("CREATE SCHEMA serendb; CREATE EXTENSION serendb WITH SCHEMA serendb")
        if ep == ep_prewarmed:
        ...
        serendb_compare.zenbenchmark.record_pg_bench_result(name, res)


def test_compare_prewarmed_read_perf(serendb_compare: SerenDBCompare):
    env = serendb_compare.env
    env.create_branch("prewarmed")
    ep_ordinary: Endpoint = serendb_compare.endpoint
    ep_prewarmed: Endpoint = env.endpoints.create_start("prewarmed")
    ...
    sql = [
        "CREATE SCHEMA serendb",
        "CREATE EXTENSION serendb WITH SCHEMA serendb",
    ...
    ep_ordinary.stop()
    ep_ordinary.start()
    with serendb_compare.record_duration("ordinary_run_duration"):
    ...
    client.prewarm_lfc_wait()
    with serendb_compare.record_duration("prewarmed_run_duration"):
...
def test_compare_prewarmed_pgbench_perf_benchmark(
    ...
    ordinary_branch_id = ""
    prewarmed_branch_id = ""
    for branch in serendb_api.get_branches(project_id)["branches"]:
    ...
    ep_ordinary = None
    ep_prewarmed = None
    for ep in serendb_api.get_endpoints(project_id)["endpoints"]:
    ...
    ordinary_uri = serendb_api.get_connection_uri(project_id, ordinary_branch_id, ordinary_id)["uri"]
    prewarmed_uri = serendb_api.get_connection_uri(project_id, prewarmed_branch_id, prewarmed_id)[
        "uri"
    ]
    ...
    def bench(endpoint_name, endpoint_id, env):
        ...
        if endpoint_name == "prewarmed":
            ...
            serendb_api.restart_endpoint(project_id, endpoint_id)
