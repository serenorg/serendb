...
def get_test_overlay_dir(request: FixtureRequest, top_output_dir: Path) -> Path:
    ...
    that a test creates. See `SerenDBEnvBuilder.overlay_mount`.
    """
...
def serendb_binpath(base_dir: Path, build_type: str) -> Iterator[Path]:
    if os.getenv("REMOTE_ENV"):
        # we are in remote env and do not have SerenDB binaries locally
    ...
    log.info(f"serendb_binpath is {binpath}")

    if not (binpath / "pageserver").exists():
        raise Exception(f"SerenDB binaries not found at '{binpath}'")
...
def compatibility_snapshot_dir() -> Iterator[Path]:
    ...
    assert compatibility_snapshot_dir_env is not None, (
        "COMPATIBILITY_SNAPSHOT_DIR is not set. It should be set to `compatibility_snapshot_pg(PG_VERSION)` path generateted by test_create_snapshot (ideally generated by the previous version of SerenDB)"
    )
...
@pytest.fixture(scope="session")
def compatibility_serendb_binpath() -> Iterator[Path | None]:
    if os.getenv("REMOTE_ENV"):
        return
    comp_binpath = None
    if env_compatibility_serendb_binpath := os.environ.get("COMPATIBILITY_SERENDB_BIN"):
        comp_binpath = Path(env_compatibility_serendb_binpath).resolve().absolute()
    yield comp_binpath
...
def test_output_dir(request: pytest.FixtureRequest, top_output_dir: Path) -> Iterator[Path]:
    ...
    for k, v in request.node.user_properties:
        # NB: the serendb_env_builder fixture uses this fixture (test_output_dir).
        # So, serendb_env_builder's cleanup runs before here.
        # The cleanup propagates SerenDBEnvBuilder.preserve_database_files into this user property.
...
def test_overlay_dir(request: FixtureRequest, top_output_dir: Path) -> Path | None:
    ...
    if os.getenv("SERENDB_ENV_BUILDER_USE_OVERLAYFS_FOR_SNAPSHOTS") is None:
        return None
    ...
    # SerenDBEnvBuilder.overlay_cleanup_teardown takes care of cleanup
