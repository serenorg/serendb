...
def test_serendb_superuser(serendb_simple_env: SerenDBEnv, pg_version: PgVersion):
    env = serendb_simple_env
    env.create_branch("test_serendb_superuser_publisher", ancestor_branch_name="main")
    pub = env.endpoints.create("test_serendb_superuser_publisher")

    env.create_branch("test_serendb_superuser_subscriber")
    sub = env.endpoints.create("test_serendb_superuser_subscriber")
    ...
    with pub.cursor(dbname="serendb", user="mr_whiskers", password="cat") as cur:
        cur.execute("SELECT pg_has_role('mr_whiskers', 'serendb_superuser', 'member')")
        assert cur.fetchall()[0][0]
        cur.execute("SELECT pg_has_role('mr_whiskers', 'serendb_superuser', 'usage')")
        assert cur.fetchall()[0][0]

        if pg_version == PgVersion.V16:
            cur.execute("SELECT pg_has_role('mr_whiskers', 'serendb_superuser', 'set')")
            assert cur.fetchall()[0][0]
    ...
    with sub.cursor(dbname="serendb", user="mr_whiskers", password="cat") as cur:
        ...
        # Test that pg_monitor is working for serendb_superuser role
        cur.execute("SELECT query from pg_stat_activity LIMIT 1")
        assert cur.fetchall()[0][0] != "<insufficient privilege>"
        # Test that pg_monitor is not working for non serendb_superuser role without grant
        cur.execute("CREATE ROLE not_a_superuser LOGIN PASSWORD 'Password42!'")
        cur.execute("GRANT not_a_superuser TO serendb_superuser WITH ADMIN OPTION")
        cur.execute("SET ROLE not_a_superuser")
        cur.execute("SELECT query from pg_stat_activity LIMIT 1")
        assert cur.fetchall()[0][0] == "<insufficient privilege>"
        cur.execute("RESET ROLE")
        # Test that pg_monitor is working for non serendb_superuser role with grant
...
def test_privileged_role_override(serendb_simple_env: SerenDBEnv, pg_version: PgVersion):
    """
    Test that we can override the privileged role for an endpoint and when we do it,
    everything is correctly bootstrapped inside Postgres and we don't have serendb_superuser
    ...
    with ep.cursor() as cur:
        ...
        cur.execute("SELECT rolname FROM pg_roles WHERE rolname = 'serendb_superuser'")
        assert len(cur.fetchall()) == 0

        cur.execute("SHOW serendb.privileged_role_name")
        assert cur.fetchall()[0][0] == PRIVILEGED_ROLE_NAME
