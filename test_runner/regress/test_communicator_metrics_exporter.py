...
SERENDB_COMMUNICATOR_SOCKET_NAME = "serendb-communicator.socket"


def test_communicator_metrics(serendb_simple_env: SerenDBEnv):
    """
    ...
    r = session.get(f"http+unix://{SERENDB_COMMUNICATOR_SOCKET_NAME}/metrics")
    ...
    with pytest.raises(
        ...
        r = session.get(f"http+unix://{SERENDB_COMMUNICATOR_SOCKET_NAME}/debug/panic")
