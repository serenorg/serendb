...
# Test that SerenDB cli is able to start and stop all processes with the user defaults.
# Repeats the example from README.md as close as it can
def test_serendb_cli_basics(serendb_env_builder: SerenDBEnvBuilder, port_distributor: PortDistributor):
    env = serendb_env_builder.init_configs()
    # Skipping the init step that creates a local tenant in Pytest tests
    try:
        env.serendb_cli.start()
        ...
        env.serendb_cli.endpoint_create(
            main_branch_name,
            pg_port,
            external_http_port,
            internal_http_port,
        ...
        env.serendb_cli.endpoint_start("ep-basic-main")

        branch_name = "migration-check"
        env.serendb_cli.timeline_branch(
            tenant_id=env.initial_tenant,
        ...
        env.serendb_cli.endpoint_start(f"ep-{branch_name}")
    finally:
        env.serendb_cli.stop()


def test_serendb_two_primary_endpoints_fail(
    serendb_env_builder: SerenDBEnvBuilder, port_distributor: PortDistributor
):
    """
    ...
    env.serendb_cli.endpoint_start("ep1")
    ...
    with pytest.raises(RuntimeError):
        assert expected_message in env.serendb_cli.endpoint_start("ep2").stderr

    env.serendb_cli.endpoint_stop("ep1")
    # ep1 is stopped so create ep2 will succeed
    env.serendb_cli.endpoint_start("ep2")
    # cleanup
    env.serendb_cli.endpoint_stop("ep2")
