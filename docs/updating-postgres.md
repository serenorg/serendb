...
1. Clone the SerenDB Postgres repository if you have not done so already.

    ```shell
    git clone git@github.com:serendb/postgres.git
    ```
...
1. Create a new branch based on the stable branch you are updating.
    ...
    ```shell
    git checkout -b my-branch-15 REL_15_STABLE_serendbdb
    ```
...
1. Push your branch to the SerenDB Postgres repository.

    ```shell
    git push origin my-branch-15
    ```

1. Clone the SerenDB repository if you have not done so already.

    ```shell
    git clone git@github.com:serendb/serendb.git
    ```
...
1. Run the SerenDB test suite to make sure that SerenDB is still good to go on this
...
1. Push the Postgres branches with the merge commits into the SerenDB Postgres repository.

    ```shell
    git push origin my-branch-15:REL_15_STABLE_serendbdb
    ```

1. Update your SerenDB PR to point at the branches.

    ```shell
    git submodule set-branch --branch REL_15_STABLE_serendbdb vendor/postgres-v15
