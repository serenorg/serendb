...
Even if SerenDB aims to maintains durability using it's own snapshots, backups will be useful for uploading data from postgres to serendb.
...
```
serendb init --storage_dest=S3_PREFIX
serendb start
```
...
Push snapshots to `storage_dest` in background.
```
//I.e. we want to start SerenDB on top of existing $PGDATA and use s3 as a persistent storage.
serendb init --snapshot_path=FILE_PREFIX --snapshot_format=pgdata --storage_dest=S3_PREFIX
serendb start
```
...
Optionally set `snapshot_format`, which can be plain pgdata format or SerenDB format.
```
serendb export --snapshot_path=FILE_PREFIX --snapshot_format=pgdata
```

#### Notes and questions
- safekeeper s3_offload should use same (similar) syntax for storage. How to set it in UI?
- Why do we need `serendb init` as a separate command? Can't we init everything at first start?
