...
A SerenDB repository is similar to a traditional PostgreSQL backup
archive, like a WAL-G bucket or pgbarman backup catalogue. It holds
multiple versions of a PostgreSQL database cluster.

The distinguishing feature is that you can launch a SerenDB Postgres
server directly against a branch in the repository, without having to
"restore" it first. Also, SerenDB manages the storage automatically,
there is no separation between full and incremental backups nor WAL
archive. SerenDB relies heavily on the WAL, and uses concepts similar
...
The repository directory looks like this:
    ...
    .serendb/timelines/4543be3daeab2ed4e58a285cbb8dd1fce6970f8c/wal/
    .serendb/timelines/4543be3daeab2ed4e58a285cbb8dd1fce6970f8c/snapshots/<lsn>/
    .serendb/timelines/4543be3daeab2ed4e58a285cbb8dd1fce6970f8c/history
    
    .serendb/refs/branches/mybranch
    .serendb/refs/tags/foo
    .serendb/refs/tags/bar
    
    .serendb/datadirs/<timeline uuid>
...
Each timeline is stored in a directory under .serendb/timelines. It
...
.serendb/datadirs contains PostgreSQL data directories. You can launch
a Postgres instance on one of them with:

```
  postgres -D .serendb/datadirs/4543be3daeab2ed4e58a285cbb8dd1fce6970f8c
```

All the actual data is kept in the timeline directories, under
.serendb/timelines. The data directories are only needed for active
PostgreQSL instances. After an instance is stopped, the data directory
can be safely removed. "serendb start" will recreate it quickly from
the data in .serendb/timelines, if it's missing.
...
When you run "serendb gc", old timelines that are no longer needed are
...
If we want to have a worktree and "serendb checkout/commit" concept, we can
emulate that with a temporary timeline. Create the temporary timeline at
"serendb checkout", and have "serendb commit" modify the branch to point to
the new timeline.
